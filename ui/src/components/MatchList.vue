    ...
    <form @submit.prevent="handleSubmit" class="space-y-4">
      <div class="grid grid-cols-2 gap-4">
        <div v-for="(team, index) in ['Team 1', 'Team 2']" :key="team">
          <h3 class="mb-2 font-semibold text-gray-900 dark:text-white">{{ team }}</h3>
          <div class="space-y-2">
            <select
      ...
      <button type="submit" class="modern-button w-full">Create Match</button>
    </form>

    <div class="space-y-4">
      <div v-for="match in matches" :key="match.id" class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
        <div class="flex justify-between items-center text-gray-900 dark:text-white">
          ...
          <div v-if="match.status === 'completed'" class="font-medium">
